...
// -----------------------------------------The Knapsack Problem, Naive Approach
...
// ----------------------------------Edit (Levenshtein) Distance, Naive Approach
function editDistanceRecursive(str1, str2, length1, length2) {
    ...
    // last chars are same, ignore last chars and count remaining
    ...
    for (var i = 0; i < length1 + 1; i++) {
        for (var j = 0; j < length2 + 1; j++) {
            ...
            if (i == 0) {
                ...
                dpMatrix[i][j] =
                    1 + Math.min(insertCost, removeCost, replaceCost);
            }
        }
    }
